...
class Attack:
    def __init__(self, model, criterion, test_criterion, norm, data_shape,
        ...
        #DAN: criterion is a <loss.VOCriterion object
    ...
    def project(self, pert, eps):
        '''DAN: this keeps the perturbation within the range [0, 1]'''
        if self.norm == 'Linf':
    ...
    def warp_pert(self, pert, perspective1, perspective2, device=None):
        '''DAN: we warp the perturbation with the homography and blur it with a box filter'''
    ...
    def test_clean_multi_input(self, eval_data_loader, eval_y_list, device):
        '''DAN: clean output is the output without the perturbation'''
        #! DAN: here we extract eval_y_list again!
        clean_output_list = []
        clean_loss_list = []
        motions_is_eval_y_counter = 9
        for data_idx, data in enumerate(eval_data_loader):
            dataset_idx, dataset_name, traj_name, traj_len, \
            img1_I0, img2_I0, intrinsic_I0, \
            img1_I1, img2_I1, intrinsic_I1, \
            img1_delta, img2_delta, \
            motions_gt, scale, pose_quat_gt, patch_pose, mask, perspective \
                = extract_traj_data(data)

            data_shape = img1_I0.shape
            dtype = img1_I0.dtype
            #DAN: in test model we use the extracted motions gt instead of eval_y_list
            # if (motions_gt != eval_y_list[data_idx]).any():
            #     print('motions_gt != eval_y_list[data_idx]')
            #     print(motions_gt)
            #     print(eval_y_list[data_idx])
            #     raise ValueError('motions_gt != eval_y_list[data_idx]')
            with torch.no_grad():
                
                clean_output, clean_loss = test_model(self.model, self.test_criterion,
                                                      img1_I0, img2_I0, intrinsic_I0,
                                                      scale, motions_gt, patch_pose,
    ...
    def compute_train_eval_clean_output(self, data_loader, y_list, eval_data_loader, eval_y_list, device=None):
        print(filename_n_line_str() + "computing clean output and clean loss over training samples")
        #DAN: thisVVVV CLEAN on train
        ...
        if eval_data_loader is None:
            print(filename_n_line_str() + "training samples will be used for evaluating perturbations")
            eval_clean_loss_list = clean_loss_list
            eval_data_loader = data_loader
            eval_y_list = y_list

        else:
            print(filename_n_line_str() + "computing clean output and clean loss over evaluation samples")
    ...
    def compute_clean_baseline(self, data_loader, y_list, eval_data_loader, eval_y_list, device=None):
        '''DAN: this computes the CLEAN output of the train and eval sets with "compute_train_eval_clean_output" 
        and then rearranges the losses'''
        #DAN: thisVVVV computes the clean flow on the training set and all of the metrics on the 
        #       eval set
        ...
        #DAN: notice that we take the data_shape 1 2 3 so that the pertubation is a single color image
        ...
        clean_loss_sum = np.sum([loss.sum().item() for loss in eval_clean_loss_list])
        ...
        return data_shape, dtype, eval_data_loader, eval_y_list, clean_flow_list, \
               ...
               best_pert, best_loss_list, clean_loss_sum, all_loss, all_best_loss
    ...
    def calc_sample_grad(self, pert, img1_I0, img2_I0, intrinsic_I0, img1_delta, img2_delta,
        ...
        #DAN: self.calc_sample_grad_aux is virtual in Attack and implemented in PGD
    ...
    def perturb_model_single(self, pert, img1_I0, img2_I0, intrinsic_I0, img1_delta, img2_delta, scale, mask1, mask2,
        ...
        '''DAN: for one trajectory, perturb the images with pert and compute the model.
                Specifics:
                warp the perturbation to the target pose, blur (warp_pert), then add the normalized 
                pertubation to the images.
                finally, run the model on the warped images'''
        pert_warp1, pert_warp2 = self.warp_pert(pert, perspective1, perspective2, device)
        
        if device is None:
            img1_adv = img1_I0.clone().detach()
            img2_adv = img2_I0.clone().detach()
            #DAN: 0 albedo is added because we start with an img1_I0 clone...
            img1_adv[mask1] += img1_delta[mask1] * pert_warp1[mask1]
            img2_adv[mask2] += img2_delta[mask2] * pert_warp2[mask2]
    ...
    def gradient_ascent_step(self, pert, data_shape, data_loader, y_list, clean_flow_list,
        ...
        '''DAN: calculate the gradient ascent step:
                the same perturbation to all the trajectories'''
        #DAN: insert perturbation into all of the images
        ...
        for data_idx, data in enumerate(data_loader):
            ...
            motions_gt, scale, pose_quat_gt, patch_pose, mask, perspective = extract_traj_data(data)
            mask1, mask2, perspective1, perspective2 = self.prep_data(mask, perspective)
            #DAN: calc sample grad is an inherited method of the Attack class, it's implemented in pgd.py
            ...
            #DAN: add all grad from the trajectory
