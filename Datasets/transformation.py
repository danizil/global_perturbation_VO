...
def pose2motion(data, skip=0):
    '''DAN: 
        input: data: 30X4X3'''
    data_size = data.shape[0] # 30 in our case...
    
    all_motion = np.zeros((data_size-1,12))
    for i in range(0,data_size-1-skip):
        #DAN: VVVV data[i,:] can already be a matrix and line2mat wont change it
        pose_curr = line2mat(data[i,:]) #DAN: pose next is 4X4
        pose_next = line2mat(data[i+1+skip,:])
        #DAN: .I is the INVERSE. to calculate the drone's motion we need the inverse of the scene motion
...
def ses2poses_quat(data):
    ...
    ses (data): N x 6
    N = 29
    quat is a scipy rotation object
    '''
    data_size = data.shape[0]
...
def SEs2ses(motion_data):
    '''DAN: takes a 30X4X3 matrix and returns a 30X6 matrix, 6 for translation and rotation vec'''
