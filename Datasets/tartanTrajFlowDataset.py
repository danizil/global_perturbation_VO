...
def get_perspective(data_size, dst_points, perspective_padding=(0, 0)):
    '''DAN: perspective is the homography of the plane of the adv patch for every frame
    ...
    #DAN: VVVVV just defines the corners of the crop of the image
    ...
    for img_points_dst in dst_points:
        ...
        points_dst = torch.FloatTensor(img_points_dst).reshape((1, -1, 2)) # (1, 4, 2) 4 points in crop
...
def kitti2SE(data):
    '''DAN: 
        returns: 4X4 matrix of (R, T)
                                0, 1
    '''
    #DAN: data comes in 1X12, reshaped to 3,4
...
def kitti_traj2SE_matrices(kitti_traj):
    '''DAN: kitti traj is the gt_poses
        returns: SEs = list([4X4, 4X4, ...])'''
    SEs = []
    for data in kitti_traj:
        SE = kitti2SE(data)
        SEs.append(SE)
        #DAN: code duplication 
        # data = data.reshape((3, 4))
        # SE = np.eye(4)
        # SE[0:3, 0:3] = data[:, 0:3]
        # SE[0:3, 3]   = data[:, -1]
...
class TrajFolderDatasetCustom(Dataset):
    ...
    def __init__(self, rootfolder, transform=None, data_size=(448, 640),
        ...
        print(filename_n_line_str() + 'Find {} image files from {} trajectories and {} datasets in root folder:{}'.format(tot_files_num, self.N,
                                                                                      self.datasets_num, rootfolder))
        print(filename_n_line_str() + "trajectories found:")
...
class MultiTrajFolderDatasetCustom(Dataset):
    ...
    def __init__(self, rootfolder, processed_data_folder="", preprocessed_data=False,
        ...
        print(filename_n_line_str() + " using custom dataset, dataset intrinsics:")
        ...
        self.rootfolder = rootfolder
        # print('VVVVVV\n384@tartanTrajFlow.py, in __init__\n^^^^^^^^^^^^')
        rootfolder_files = listdir(rootfolder)
        if processed_data_folder is None:
            #DAN: if data isn't preprocessed, make the directory for processing. if called from 
            # utils.py, should not be entered
        ...
        if not isdir(processed_data_folder):
            ...
            print(filename_n_line_str() + " processing data folder: " + str(processed_data_folder))
        elif not preprocessed_data:
            rmtree(processed_data_folder)
            mkdir(processed_data_folder)
            print(filename_n_line_str() + " processing data folder, previously processed data will be deleted: " + str(processed_data_folder))
        else:
            print(filename_n_line_str() + " using preprocessed data folder: " + str(processed_data_folder))
        ...
        self.traj_len = None
        tot_files_num = 0
        # a dataset is 0,1,2,3,4
        # every dataset has trajectorries 00000_seed_0000
        for dataset_idx, dataset_name in enumerate(self.datasets_names):
            ...
            #! VVVVV  saving all of the data unless told otherwise, and there is saved data VVVVVV
            t = time.time()
            for traj_idx, traj_folder in enumerate(dataset_traj_folders):
                ...
                if self.traj_len is None:
                    ...
                    assert self.traj_len == traj_len
                tot_files_num += traj_len
            print(filename_n_line_str() + f'after processing {traj_folder}, time: {time.time() - t}')
            #! ^^^^^^ MAJOR TIME CONSUMING STEP ^^^^^^
            self.processed_traj_folders += processed_dataset_traj_folders


        print(filename_n_line_str() + 'Find {} trajectories within {} datasets in root folder:{}'.format(self.N, self.datasets_num, rootfolder))
        print(filename_n_line_str() + 'Each trajectory contains {} images, and {} images in total within {} trajectories'.format(self.traj_len, tot_files_num, self.N))
    ...
    def process_imgfiles(self, img_files):
        ...
        intrinsic_list = []
        for img1_idx, imgfile1 in enumerate(img_files[:-1]):
            #DAN: goes over consecutive images in a trajectory
            imgfile1 = imgfile1.strip()
            imgfile2 = img_files[img1_idx + 1].strip()
            img1 = cv2.cvtColor(cv2.imread(imgfile1), cv2.COLOR_BGR2RGB) #DAN: images are coded with bgr
            img2 = cv2.cvtColor(cv2.imread(imgfile2), cv2.COLOR_BGR2RGB)
    ...
    def process_trajectory_folder(self, traj_folder, preprocessed_data):
        ...
        I0files = [(traj_folder + '/' + ff) for ff in rgbfiles if (ff.startswith('I0'))]
        I0files.sort()
        # input(f'\nVVVVVVVVVV\n@560tratanTrajFlowDataset \n check that I0 files are the names of the images  \n^^^^^^^^^^^')
        I1files = [(traj_folder + '/' + ff) for ff in rgbfiles if (ff.startswith('I1'))]
        I1files.sort()
        traj_len = len(I0files)
        if traj_len > self.max_traj_len:
        ...
        #DAN: seems like the intrinsic is the same for all frames in a DATASET (let alone a trajectory)
        img1_I0_traj_list, img2_I0_traj_list, intrinsic_I0_traj_list = self.process_imgfiles(I0files)
        img1_I1_traj_list, img2_I1_traj_list, intrinsic_I1_traj_list = self.process_imgfiles(I1files)
...
class MultiTrajFolderDatasetRealData(Dataset):
    ...
    def __init__(self, rootfolder, processed_data_folder="", preprocessed_data=False,
        ...
        if not isdir(processed_data_folder):
            preprocessed_data = False
            mkdir(processed_data_folder)
            print("@tratanTrajFlow697: processing data folder: " + str(processed_data_folder))
        elif not preprocessed_data:
            rmtree(processed_data_folder)
            mkdir(processed_data_folder)
            print("@tratanTrajFlow697: processing data folder, previously processed data will be deleted: " + str(processed_data_folder))
        else:
            print("@tratanTrajFlow697: using preprocessed data folder: " + str(processed_data_folder))
        ...
        input(f'\nVVVVVVVVVV\n@716 tartan: is preprocessed data? {preprocessed_data} \n^^^^^^^^^^^')
        if preprocessed_data:
        ...
        print('@tratanTrajFlow780: processed data folder:{}, contains {} trajectories within {} datasets'.format(processed_data_folder, self.N, self.datasets_num))
        print('@tratanTrajFlow781: Each trajectory contains {} images, and {} images in total within {} trajectories'.format(self.traj_len, tot_files_num, self.N))

        print("@tratanTrajFlow780: trajectories found:")
        print(self.traj_names)
    ...
    def process_and_save_trajectory_folder(self, traj_folder, processed_traj_folder, preprocessed_data):
        ...
        intrinsic images are all the same for the same dataset and are a grid that depends on camera stuff'''
        traj_len, img1_I0, img2_I0, intrinsic_I0, img1_I1, img2_I1, intrinsic_I1, \
        motions, scales, poses_quat, patch_rel_pose, perspective, mask = self.process_trajectory_folder(traj_folder, preprocessed_data)
        if not preprocessed_data:
            torch.save(img1_I0, processed_traj_folder + '/img1_I0.pt')
    ...
    def process_trajectory_folder(self, traj_folder, preprocessed_data):
        '''DAN: this function takes the raw data from the traj folder and makes it into pt tensors
        called by process_and_save_trajectory_folder'''
        files = listdir(traj_folder)
        rgbfiles = [ff for ff in files if (ff.endswith('.png') or ff.endswith('.jpg'))]
        I0files = [(traj_folder + '/' + ff) for ff in rgbfiles if (ff.startswith('I0'))]
        I0files.sort()
        ...
        if traj_len > self.max_traj_len:
            ...
            traj_len = len(I0files)
        if preprocessed_data:
            #DAN: no reason to go in here because processed data was checked before, here must be None
        ...
        while True:
            inpt = input(f'\nVVVVVVVVVV\n@902tartanTraj gt poses shape {gt_poses.shape} \n^^^^^^^^^^^')
            if inpt == 'fin':
                break
            eval('print(' + inpt + ')')
        #DAN: gt_poses is kitty_traj 30X12, what is 30?
        
        gt_poses_shifted = np.array(kitti_traj2SE_matrices(gt_poses)) # 30X4X4 array
        matrix = pose2motion(gt_poses_shifted[:, 0:3]) # in: [30,3,4] out [29,12] - the relative motion of the drone between frames 
        motions = SEs2ses(matrix).astype(np.float32) #DAN: out: [29, 6] 6 is [translation, rotation]
        scales = torch.tensor(np.linalg.norm(motions[:, :3], axis=1))
        poses_quat = ses2poses_quat(motions)
        assert (len(motions) == len(I0files) - 1)
        ...
        mask_coords = np.genfromtxt(mask_coords_path, delimiter=',')
        # we want point order topLeft -> clockwise, but have bl->br->tl->tr, so:
        #DAN: every pair of pts is the xy of the corner pixel of the mask that's what he explains here^
        points = [np.array([[pts[4], pts[5], pts[6], pts[7]], [pts[2], pts[3], pts[0], pts[1]]], dtype=np.int32)
                  for pts in mask_coords]
        if len(points) > self.max_traj_len:
            points = points[:self.max_traj_len]
        #DAN: self.data_size is hw of the images
        perspective = get_perspective(self.data_size, points, self.perspective_padding)
